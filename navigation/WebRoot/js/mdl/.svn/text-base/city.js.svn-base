 ...
 * 
 * @version 2.0.7.beta, 2009-11-16 19:48:29 by Fuchun.
 * Depends on jQuery 1.3.x
 * Depends on GUI 2.0.x
 */
 ...
 * </pre>
 * 
 * @version 2.0.7.beta, 2009-11-16 19:48:29
 * @author  wenjian
 */
 
if(typeof depiao == 'undefined') { depiao = {}; }
if(typeof depiao.ui == 'undefined') { depiao.ui = {}; } 
(function($) {


/**
 * 新建一个城市级联列表控件。(暂不支持，区县)
 * @class depiao.ui.City 城市级联列表控件。
 * @example
 ...
 * @returns {depiao.ui.City} 返回新的城市级联列表控件实例。
 */
 
var data = {"provinces":[{"id":11,"name":"北京","citys":[{"1":"东城区"},{"2":"西城区"},{"3":"崇文区"},{"4":"宣武区"},{"5":"朝阳区"},{"6":"丰台区"},{"7":"石景山区"},{"8":"海淀区"},{"9":"门头沟区"},{"11":"房山区"},{"12":"通州区"},{"13":"顺义区"},{"14":"昌平区"},{"15":"大兴区"},{"16":"怀柔区"},{"17":"平谷区"},{"28":"密云县"},{"29":"延庆县"}]},
	...
	/**@#+
	 * @memberOf depiao.ui.City
	 * @method
	 */
	depiao.ui.City.prototype = {
	    // 初始化城市级联列表控件
	    initComponent: function(options, defaults) {
	        ...
	        for(var i=0; i<provinces.length; i++){
				$self.jProvince.addItem({text:provinces[i].name, value:provinces[i].id});
			}
			//添加省选择事件
			this.jProvince.addEvent("change", function() {
				$self.jCity.remove();
				$self.jCity.addItem({text:"市", value:"0"});
				var citys = getCitys(this.value);
				//构建市列表
		        for(var i=0; i<citys.length; i++){
			        for(var prop in citys[i]){  
			            $self.jCity.addItem({text:citys[i][prop], value:prop});
			            break;
			        } 					
				}
				//设置默认值
				$self.jCity.select($self.defaults.city);
			});
			//设置默认值
			$self.jProvince.select($self.defaults.province, function(){
				$(this).change();
			});
	    }
	};
	...
	/** 根据省份ID,返回省份名称 */
	depiao.ui.City.getProvinceText = function(pid){
        var provinces = data.provinces;
        for(var i=0; i<provinces.length; i++){
	...
	/** 根据省份，市ID返回，市名称 */
	depiao.ui.City.getCityText = function(pid, cid){
