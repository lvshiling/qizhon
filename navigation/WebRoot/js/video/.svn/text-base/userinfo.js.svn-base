...
cityArray[14] = new Array("江西省","南昌市|景德镇|九江|鹰潭|萍乡|新余|赣州|吉安|宜春|抚州|上饶"); 
...
function getCity(currProvince){
	if(currProvince!=="" && currProvince!=0){
		var currProvId = parseInt(currProvince);
		var i = parseInt(currProvId/100) - 1; //获取当前省坐标
		var tmpCityArray = cityArray[i][1].split("|"); //得到当前省所辖的地市
		var selCity = document.getElementById("selCity");
		selCity.length = 0 ; //清空城市下拉框
		for(var j=0; j<tmpCityArray.length; j++){
			selCity.options[j] = new Option(tmpCityArray[j], currProvId+(j+1));
		}
	}
}
function getCityText(cityId){
	if(cityId!="" && cityId.length==3){
		...
		var j = cid % 100 - 1;
		return cityArray[i][1].split("|")[j];
	}
	return "";
}
function getProvinceText(provId){
	var text = "";
	if(provId!=='' && provId!=0){
		var i = provId / 100 - 1;
		text = cityArray[i][0];
