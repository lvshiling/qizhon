...
function getNeteasyReply(id,el){			//读取腾讯评论
	global_replayPage["neteasy_"+id] ++;
	//初始化页数
	if(el == null){					//初次获取评论
		global_replayPage["neteasy_"+id] = 1;
		$("#neteasyReplyList_"+id).html('');
	}else $(el).parent().hide();	//下一页的情况
	
	if(id != null && id != ''){
		$("#neteasyReplyList_"+id).append(globla_loading).parent().show();
		var sinceId = '';
		var timeobj = $(".reply_time_"+id);
		if(timeobj.length > 0) sinceId=timeobj[timeobj.length-1].value;
		$.post("${domain }/weibo/ajaxGetNeteasyReplyPage.do", {"sid":id,"p":global_replayPage["neteasy_"+id],"target":sinceId}, function(d){
			$(".loading").remove();
			$("#neteasyReplyList_"+id).append(d);
		},'html');
	}
}
$(function(){
