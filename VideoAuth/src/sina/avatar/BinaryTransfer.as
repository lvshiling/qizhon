			...
			switch (event.type) {
				case Event.COMPLETE:
					...
					try {
						...
						MonsterDebugger.trace('binary transfer raw', event.target.data);
						MonsterDebugger.trace('binary transfer json', this._json);
						this._responseText = event.target.data;
						this.dispatchEvent(new Event(COMPLETE));
					} catch (e:JSONParseError) {
						this._json = {};
						MonsterDebugger.trace('binary transfer error', event.target.data);
						this._responseText = event.target.data;
